...
- **Acceptance:**
  ...
  - [x] On successful completion the last live message ends with a terminal marker (`✓`). Evidence: `engine/tg/streamer.ts::OK_MARKER`; test `engine/tg/streamer_test.ts::LiveHandle.finalize(ok) flushes and appends success marker`.
  ...
  - [x] `Dispatcher` pipes `onOutput`/`onEvent` from `ai-ide-cli` into the live message, with a fallback batched path when no `Streamer` is configured. Evidence: code `// FR-EVENT-STREAM` in `engine/dispatcher.ts::#runIdeStreamed`; test `engine/dispatcher_test.ts::Dispatcher streams onOutput lines through Streamer to TG edits`.
