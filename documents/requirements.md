...
- **Acceptance:**
  ...
  - [x] `/model <value>`, `/effort <value>`, `/perm <value>` validate against per-IDE whitelists and reject unknown values without mutating state. Evidence: tests `…Dispatcher /model opus stores setting`, `…Dispatcher /model with invalid value rejects and preserves state`, `…Dispatcher /effort on opencode reports not supported`, `…Dispatcher /effort high on codex stores setting`.
  - [x] `effort` is forwarded to the runtime via `engine/effort.ts::effortToInvokeFields`: Claude → raw `extraArgs: { "--effort": <val> }` (preserves `xhigh`/`max` outside the typed enum); Codex → typed `reasoningEffort` (mirrors `ai-ide-cli`'s `ReasoningEffort` enum); OpenCode/Cursor → ignored (whitelist empty). Evidence: tests `…Dispatcher applies stored settings to IDE invocation` (Claude `extraArgs`), `…Dispatcher omits --effort for non-claude ides` (asserts both `extraArgs` and `reasoningEffort` undefined for OpenCode), `…Dispatcher maps effort to typed reasoningEffort for codex`, `engine/ide_session_test.ts::SessionManager passes typed reasoningEffort for codex on openSession`, `…SessionManager keeps --effort extraArgs for claude on openSession`, `engine/settings_test.ts::validateEffort accepts codex whitelist (typed ReasoningEffort)`.
...
### 3.9 FR-CAPABILITY-INVENTORY
- **Desc:** Daemon discovers IDE skills and slash commands and exposes them as Telegram bot commands so the TG client's `/`-menu lists every capability available in the project. Discovery is on-demand via `/refresh`; results are cached on disk for restart-cheap reuse. Reserved daemon commands keep priority; collisions are dropped with a warning.
- **Scenario:** User sends `/refresh` → daemon calls `RuntimeAdapter.fetchCapabilitiesSlow` (one LLM turn) → sanitizes names to TG's `^[a-z0-9_]{1,32}$` regex → drops reserved-collision and post-sanitize duplicates → writes `.tg-ide-bridge/capabilities.json` → reissues `setMyCommands` with reserved + discovered → replies a summary `discovered N (skipped M: <reasons>)`. Subsequent restarts read the cache and reissue `setMyCommands` without re-fetching. When user types a discovered command in TG (e.g. `/flowai_skill_x args`), the daemon rewrites it to the original IDE name (`/flowai-skill-x args`) and forwards it as a normal IDE prompt.
- **Acceptance:**
  - [x] `/refresh` calls `fetchCapabilitiesSlow` only when `capabilities.capabilityInventory === true`; otherwise replies `not supported on ide=<id>` and does not mutate state. Evidence: code `// FR-CAPABILITY-INVENTORY` in `engine/dispatcher.ts::#handleRefresh`; tests `engine/dispatcher_test.ts::Dispatcher /refresh replies 'not supported' when ide lacks inventory`, `…runs provider.refresh when supported and replies summary`.
  - [x] Discovered names are sanitized to TG's bot-command regex; entries that sanitize to a reserved-daemon name or to a duplicate of another entry are dropped, and the `/refresh` reply lists how many were skipped and why. Evidence: code `// FR-CAPABILITY-INVENTORY` in `engine/capabilities.ts` (`sanitizeName`, `buildRegistry`); tests `engine/capabilities_test.ts::buildRegistry sanitizes and orders commands before skills`, `…drops post-sanitize duplicates`, `…caps total at 100 - reserved.size`, plus dispatcher summary test.
  - [x] Inventory is persisted as `.tg-ide-bridge/capabilities.json` (atomic write, mode 0600); on startup the daemon reads it and registers the merged set via `setMyCommands` without an LLM call. Evidence: code `// FR-CAPABILITY-INVENTORY` in `engine/capabilities.ts::saveRegistry`/`loadRegistry`, `engine/cli.ts` (`loadRegistry` + `mergeCommandList` before `setMyCommands`); tests `engine/capabilities_test.ts::saveRegistry writes the file and loadRegistry reads it back`, `…writes file with mode 0600 (POSIX)`.
  - [x] Discovered TG commands are forwarded to the IDE with the original (un-sanitized) name preserved as the prompt's leading slash command. Evidence: code `// FR-CAPABILITY-INVENTORY` in `engine/dispatcher.ts::#rewriteDiscovered`; test `engine/dispatcher_test.ts::Dispatcher rewrites discovered TG name to original IDE name`.
  - [x] Reserved daemon commands (`/reset`, `/stop`, `/settings`, `/model`, `/effort`, `/perm`, `/timeout`, `/retries`, `/retry_delay`, `/refresh`) keep their behavior; unknown `/`-prefixed messages not in the registry are forwarded verbatim (no regression vs. current). Evidence: existing `engine/dispatcher_test.ts` reserved-command tests; new test `…forwards unknown /<cmd> verbatim when not in registry`.
